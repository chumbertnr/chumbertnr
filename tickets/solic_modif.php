...
if (isSet($_POST['guardar']) and strlen($_POST['comentario'])>0)
        {$fecha=fecha_mysql(date("$Date_format H:i:s"));
         $comentario=mysql_real_escape_string(html_entity_decode($_POST['comentario'],ENT_QUOTES));
...
elseif ($estado=="PAS")
     {$fecha=fecha_mysql(date("$Date_format H:i:s"));
      ...
      $adjunto = mysql_real_escape_string(html_entity_decode($adjunto,ENT_QUOTES));
      $telefono=mysql_real_escape_string(html_entity_decode($telefono,ENT_QUOTES));
      $ape_y_nom=mysql_real_escape_string(html_entity_decode($ape_y_nom,ENT_QUOTES));
      $incidente=mysql_real_escape_string(html_entity_decode($incidente,ENT_QUOTES));
